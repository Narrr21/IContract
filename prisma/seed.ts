...
async function main() {
  ...
  const partnershipContracts = createdContracts.filter(c => c.type === 'partnership');
  ...
  const partnershipDetails = [
    // Partnership 1: ABC Corporation
    {
      ...
      desklayanan: 'Web and mobile application development, cloud migration services, digital transformation consulting',
      ...
      syaratlayanan: 'Monthly progress reports, quarterly business reviews, annual audits',
      ...
      majeure: 'Force majeure includes natural disasters, government actions, pandemic, war, and other unforeseeable circumstances'
    },
    ...
    // Partnership 2: PT Teknologi Maju
    {
      ...
      desklayanan: 'ERP implementation, cloud infrastructure setup, AI/ML solutions, cybersecurity services',
      ...
      syaratlayanan: 'Weekly status updates, monthly steering committee meetings, quarterly performance reviews',
      nominal: 5000000000, // 5 billion IDR
      tenggatbayar: new Date('2024-03-01'),
      syaratbayar: 'Milestone-based payments, 50% upfront, 30% mid-project, 20% completion',
      bank: 'Bank Central Asia (BCA)',
      namapemilik: 'Your Company Ltd',
      norek: '9876543210987',
      denda: '1.5% per month for delayed milestones, professional mediation required after 60 days',
      ...
      majeure: 'Extended force majeure clause including cyber attacks, regulatory changes, economic crisis'
    },
    ...
    // Partnership 3: Synergy Industries (Draft)
    {
      ...
      desklayanan: 'Product manufacturing, supply chain management, distribution network, quality control',
      ...
      syaratlayanan: 'Bi-weekly production reports, monthly quality audits, quarterly board meetings',
      nominal: 7500000000, // 7.5 billion IDR
      tenggatbayar: new Date('2024-05-01'),
      syaratbayar: 'Quarterly payments based on production volumes, LC payment terms',
      bank: 'Bank Rakyat Indonesia (BRI)',
      namapemilik: 'Your Company Ltd',
      norek: '5432109876543',
      denda: '3% per month for production delays, penalty clauses for quality issues',
      tenggatklaim: new Date('2024-10-01'),
      makskompensasi: 1500000000, // 1.5 billion IDR
      sengketa: 'Surabaya Commercial Court, alternative dispute resolution preferred',
      majeure: 'Manufacturing disruption clause, supply chain interruption, regulatory compliance issues'
    }
  ];
  ...
  console.log(`🤝 Partnership details created: ${partnershipDetails.length}`);
  console.log('\n📈 Contract breakdown:');
  console.log(`  - Partnership: ${createdContracts.filter(c => c.type === 'partnership').length}`);
  console.log(`  - Employment: ${createdContracts.filter(c => c.type === 'employment').length}`);
  console.log(`  - Services: ${createdContracts.filter(c => c.type === 'services').length}`);
  console.log(`  - Consulting: ${createdContracts.filter(c => c.type === 'consulting').length}`);
  console.log('\n🎯 Status breakdown:');
  console.log(`  - Active: ${createdContracts.filter(c => c.status === 'aktif').length}`);
  console.log(`  - Draft: ${createdContracts.filter(c => c.status === 'draft').length}`);
  console.log(`  - Terminated: ${createdContracts.filter(c => c.status === 'dihentikan').length}`);
  console.log(`  - Expired: ${createdContracts.filter(c => c.status === 'berakhir').length}`);
